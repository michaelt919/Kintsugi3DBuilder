...
namespace Microsoft.Samples.Kinect.KinectFusionExplorer
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged, IDisposable
    {
        ...
        private void TrackCamera()
        {
            ...
            if (trackingSucceeded && (saveViewThread == null || !saveViewThread.IsAlive))
            {
                if (AddViewSetEntry(calculatedCameraPos)) // Only save the image if the entry was successfuly added to the vset file
                {
                    logFileStreamWriter.WriteLine("\tAlignment energy: " + this.alignmentEnergy);
                    this.viewSetSize++;
                    canProceed = false;
                            ...
                            y =>
                            {
                                ...
                                for (int x = 0; x < this.depthWidth; ++x, ++destIndex)
                                {
                                    ...
                                    if (colorInDepthX >= 0 && colorInDepthX < this.colorWidth && colorInDepthY >= 0
                                        && colorInDepthY < this.colorHeight && depthImagePixels[destIndex] != 0)
                                    {
            ...
            lock (rawDataLock)
            {
                ...
                y =>
                {
                    ...
                    for (int x = 0; x < this.colorWidth; ++x, ++index)
                    {
                        ...
                        this.colorFramePixelsArgb[index] = (255 << 24) | (colorImagePixels[bufferIndexBase + 2] << 16) | (colorImagePixels[bufferIndexBase + 1] << 8) | colorImagePixels[bufferIndexBase + 0]; // set blue, green, red
                    }
                });
            }
        ...
        private void SaveLightFieldView()
        {
            if (this.captureColor)
            {
                ...
                lock (this.rawDataLock)
                {
                    ...
                    colorBufferCopy = new byte[colorImagePixels.Length];
                    Array.Copy(colorImagePixels, colorBufferCopy, colorImagePixels.Length);
