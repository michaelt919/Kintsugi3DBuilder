...
vec3 rand3(vec2 n)
{
    return vec3(rand(n), rand(n + vec2(2, 0)), rand(n + vec2(0, 2)));
}
...
void main()
{
    ...
    vec3 originalLinear = (diffuseColor * l.nDotL / PI + specular) * incidentRadiance;

    // Reflectance is implicitly multiplied by n dot l.
    // Gamma correction intentionally omitted for error calculation.
    fragColor = vec4(linearizeColor(tonemapFromLuminanceMap(originalLinear) + (rand3(floor(gl_FragCoord.xy)) - 0.5) * noiseScale) / incidentRadiance, 1.0);
}
