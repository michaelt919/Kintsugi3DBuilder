...
void main()
{
    ...
    vec3 originalLinear = diffuseColor * l.nDotL / PI + specular;

    // Reflectance is implicitly multiplied by n dot l.
    // Gamma correction intentionally omitted for error calculation.
    fragColor = vec4(sRGBToLinear(linearToSRGB(originalLinear) + (rand3(floor(gl_FragCoord.xy)) - 0.5) * noiseScale), 1.0);
}
