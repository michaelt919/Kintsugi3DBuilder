...
final class ReflectanceMatrixBuilder
{
     ...
     */
    ReflectanceMatrixBuilder(float[] colorAndVisibility, float[] halfwayAndGeom, SpecularFitSolution solution, SimpleMatrix contributionATA,
    ...
    private void processSample(int p)
    {
        ...
        double mExact = halfwayAndGeom[4 * p] * settings.microfacetDistributionResolution;
        ...
        for (int b1 = 0; b1 < settings.basisCount; b1++)
        {
            ...
            double weightedReflectanceRed   = solution.getWeights(p).get(b1) * addlWeightSquared * colorAndVisibility[4 * p];
            double weightedReflectanceGreen = solution.getWeights(p).get(b1) * addlWeightSquared * colorAndVisibility[4 * p + 1];
            double weightedReflectanceBlue  = solution.getWeights(p).get(b1) * addlWeightSquared * colorAndVisibility[4 * p + 2];
    ...
    private void validate()
    {
        ...
        SimpleMatrix mA = new SimpleMatrix(colorAndVisibility.length / 4, settings.basisCount * (settings.microfacetDistributionResolution + 1), DMatrixRMaj.class);
        ...
        for (int p = 0; p < colorAndVisibility.length / 4; p++)
        {
            if (colorAndVisibility[4 * p + 3] > 0)
            {
                ...
                {
                    // diffuse
                    mA.set(p, b, halfwayAndGeom[4 * p + 2] * solution.getWeights(p).get(b) * diffuseFactor);
                    ...
                    if (mExact < settings.microfacetDistributionResolution)
                    {
                        ...
                        mA.set(p, j, t * halfwayAndGeom[4 * p + 2] * halfwayAndGeom[4 * p + 1] * solution.getWeights(p).get(b));

                        for (int m = mFloor + 1; m < settings.microfacetDistributionResolution; m++)
                        {
                            j = settings.basisCount * (m + 1) + b;
                            // specular (no blending)
                            mA.set(p, j, halfwayAndGeom[4 * p + 2] * halfwayAndGeom[4 * p + 1] * solution.getWeights(p).get(b));
                        }
                    }
                }
            }
