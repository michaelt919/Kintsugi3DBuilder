...
public class TextureFitExecutor<ContextType extends Context<ContextType>>
{
    ...
    private static class DiffuseFit<ContextType extends Context<ContextType>>
    {
        ...
        void fitImageSpace(Texture<ContextType> viewImages, Texture<ContextType> depthImages, Texture<ContextType> shadowImages, ParameterizedFitBase.SubdivisionRenderingCallback callback)
            throws IOException
        {
            base.fitImageSpace(framebuffer, viewImages, depthImages, shadowImages, callback);
        }

        void fitTextureSpace(File preprocessDirectory,  ParameterizedFitBase.SubdivisionRenderingCallback callback) throws IOException
        {
    ...
    private static class SpecularFit<ContextType extends Context<ContextType>>
    {
        ...
        void fitImageSpace(Texture<ContextType> viewImages, Texture<ContextType> depthImages, Texture<ContextType> shadowImages,
                Texture<ContextType> diffuseEstimate, Texture<ContextType> normalEstimate, ParameterizedFitBase.SubdivisionRenderingCallback callback) throws IOException
        {
        ...
        void fitTextureSpace(File preprocessDirectory, Texture<ContextType> diffuseEstimate, Texture<ContextType> normalEstimate,
                ParameterizedFitBase.SubdivisionRenderingCallback callback)
            throws IOException
        {
            base.drawable.program().setTexture("diffuseEstimate", diffuseEstimate);
            base.drawable.program().setTexture("normalEstimate", normalEstimate);
    ...
    private static class AdjustFit<ContextType extends Context<ContextType>>
    {
        ...
        void fitImageSpace(Framebuffer<ContextType> framebuffer, Texture<ContextType> viewImages, Texture<ContextType> depthImages, Texture<ContextType> shadowImages,
                ...
                Texture<ContextType> peakEstimate, Texture<ContextType> errorTexture, ParameterizedFitBase.SubdivisionRenderingCallback callback) throws IOException
        {
            base.drawable.program().setTexture("diffuseEstimate", diffuseEstimate);
            base.drawable.program().setTexture("normalEstimate", normalEstimate);
        ...
        void fitTextureSpace(Framebuffer<ContextType> framebuffer, File preprocessDirectory,
                ...
                Texture<ContextType> errorTexture, ParameterizedFitBase.SubdivisionRenderingCallback callback) throws IOException
        {
            base.drawable.program().setTexture("diffuseEstimate", diffuseEstimate);
            base.drawable.program().setTexture("normalEstimate", normalEstimate);
            base.drawable.program().setTexture("specularEstimate", specularEstimate);
...
//        final int DEBUG_PIXEL_Y = param.getTextureSize() - 365;
    ...
    private void darpaTestSuite(float avgDistance) throws IOException
    {
        ...
        {
            modifyLightOffset(new Vector2(offset * 0.01f * avgDistance, 0.0f));
            fitAndSaveTextures(new File(outputDir, String.format("%04d_lightHorizontal_" + offset, index)), null);
            index++;
        }
        ...
        for (int offset = -lightMax; offset <= lightMax; offset += lightIncr)
        {
            ...
            fitAndSaveTextures(new File(outputDir, String.format("%04d_lightVertical_" + offset, index)), null);
            index++;
        }
        ...
        for (int angle = -normalMax; angle <= normalMax; angle += normalIncr)
        {
            ...
            fitAndSaveTextures(new File(outputDir, String.format("%04d_normalParallel_" + angle, index)),
        ...
        {
            float radianAngle = (float)(angle * Math.PI / 180);
            fitAndSaveTextures(new File(outputDir, String.format("%04d_normalOrthogonal_" + angle, index)),
                new Vector3((float)Math.sin(radianAngle) * sqrtHalf, (float)Math.sin(radianAngle) * sqrtHalf, (float)Math.cos(radianAngle)));
            index++;
        }
    }
    ...
    private void fitAndSaveTextures(File instanceDirectory, Vector3 normalDirectionOverride) throws IOException
    {
        ...
        peakIntensityEstimator.estimate(param.getTextureSize(), param.getTextureSize(), 2.0f, 0.02f);
