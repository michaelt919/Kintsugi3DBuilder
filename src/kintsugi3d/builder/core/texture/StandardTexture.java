...
{
    DIFFUSE_COLOR(new NamedTextureInfo("diffuse", "Diffuse map",
    "The RGB color of the surface's Lambertian diffuse reflectance.  Used primarily in the reflectivity and basis material representations.")),
    SPECULAR_COLOR(new NamedTextureInfo("specular", "Specular map",
        "The RGB color (F0) of the surface's Cook-Torrance / Schlick Fresnel specular reflection.  Used primarily in the reflectivity material representation to represent the color of specular highlights and reflections.")),
    NORMAL_MAP(new NamedTextureInfo("normal", "Normal map",
    ...
    ROUGHNESS(new NamedTextureInfo("roughness", "Roughness map",
        "The GGX microfacet roughness of the surface: Black (0) means mirror-like reflection, white (1) means fully diffuse (albeit with some directionality).  Used primarily in the metallicity and reflectivity material representations to approximate the microfacet distribution.")),
    ALBEDO(new NamedTextureInfo("albedo", "Albedo map",
        "The RGB albedo color that combines both diffuse and specular color.  Used primarily in the metallicity material representation to represent color.  Intended for PBR rendering pipelines that don't support separate diffuse and specular textures.")),
    OCCLUSION(new NamedTextureInfo("occlusion", "Occlusion map",
    ...
    METALLIC(new NamedTextureInfo("metallic", "Metallic map",
    ...
    ORM(new NamedTextureInfo("orm", "ORM map",
    ...
    ERROR(new NamedTextureInfo("error", "Error map",
    ...
    public final TextureInfo details;

    StandardTexture(TextureInfo details)
    {
        this.details = details;
    }

    public static <TextureType> Map<TextureInfo, TextureType> convertEnumMapToObjectMap(Map<StandardTexture, TextureType> original)
    {
        return original.entrySet().stream().collect(Collectors.toMap(
            entry -> entry.getKey().details, Entry::getValue));
    }

    public static <TextureType> Map<StandardTexture, TextureType> convertObjectMapToEnumMap(Map<TextureInfo, TextureType> original)
    {
