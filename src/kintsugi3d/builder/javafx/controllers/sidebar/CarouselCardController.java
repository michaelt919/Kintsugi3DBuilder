...
public class CarouselCardController {
    ...
    public void initData(UserShader shader) {
        ...
        */
        Global.state().getUserShaderModel().registerHandler(newShader -> {
            updateCheckboxState(newShader);
        });
    ...
    private void updateCheckboxState(UserShader activeShader) {
        ...
        if (currentShader != null && currentShader.equals(activeShader)) {
            textureType.setSelected(true);
        } else {
            textureType.setSelected(false);
        }
    }
