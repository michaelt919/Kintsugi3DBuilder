...
// STATIC FIELDS
    ...
    public static PLY read(InputStream in) throws IOException
    {
        ...
        while (true)
        {
            ...
            while (true)
            {
                if ((match = MATCH_ELEMENT.matcher(line)).matches())
                {
                    ...
                    {
                        line = header.readLine();
                        if ((match = MATCH_COMMENT.matcher(line)).matches())
                        {
                            result.comments.put(header.lineCount(), match.group("COMMENT"));
                        }
                        else if (!(match = MATCH_PROPERTY.matcher(line)).matches())
                        {
                            continue parse_line;
                        }
                        else
                ...
                else if (MATCH_END.matcher(line).matches())
                {
                    break parse_header;
                }
                else if ((match = MATCH_COMMENT.matcher(line)).matches())
                {
                    result.comments.put(header.lineCount(), match.group("COMMENT"));
                }
                else if ((match = MATCH_OBJ_INFO.matcher(line)).matches())
                {
                    result.obj_infos.put(header.lineCount(), match.group("OBJINFO"));
                }
                else
                {
        ...
        {
            case ASCII:
                input = new DataStreamInASCII(header);
                break;
            case BINARY_BIG_ENDIAN:
                input = new DataStreamInBE(in);
                break;
            case BINARY_LITTLE_ENDIAN:
                input = new DataStreamInLE(in);
                break;
            default:
    ...
    public static PLY loadFromZip(File zipFolder, String targetFileName) throws IOException
    {
        ...
        try (InputStream in = new FileInputStream(zipFolder))
        {
            return loadFromZip(in, targetFileName);
        }
    }
     ...
     * Loads a PLY file from inside a zip file.
     *
     * @param inputStream    The stream containing the zip file
     * @param targetFileName The name of the PLY file that is zipped inside zip file
     *
     * @return PLY object
     *
     * @throws IOException
     */
    public static PLY loadFromZip(InputStream inputStream, String targetFileName) throws IOException
    {
    ...
    public final Map<String, PLYElementList> elements = new LinkedHashMap<String, PLYElementList>()
    {
        ...
        @Override
        public PLYElementList put(String key, PLYElementList val)
        {
...
// METHODS
     ...
     */
    public void setFormat(PLYFormat _format)
    {
        format = requireNonNull(_format);
    }
    ...
    public void write(OutputStream out) throws IOException
    {
        ...
        {
            case ASCII:
                output = new DataStreamOutASCII(out);
                break;
            case BINARY_BIG_ENDIAN:
                output = new DataStreamOutBE(out);
                break;
            case BINARY_LITTLE_ENDIAN:
                output = new DataStreamOutLE(out);
                break;
            default:
                throw new NullPointerException();
        }
